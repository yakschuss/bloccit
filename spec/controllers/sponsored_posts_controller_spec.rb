...
RSpec.describe SponsoredPostsController, type: :controller do
  ...
  describe "POST #create" do
    ...
    it "increases the number of SponsoredPost by 1" do
      expect{post :create, topic_id: my_topic.id, sponsored_post: {title: RandomData.random_sentence, body: RandomData.random_paragraph, price: rand(0..2)}}.to change(SponsoredPost, :count).by(1)
    end

    it "assigns the new post to @sponsored_post" do
      post :create, topic_id: my_topic.id, sponsored_post: {title: RandomData.random_sentence, body: RandomData.random_paragraph, price: rand(0..2)}
      expect(assigns(:sponsored_post)).to eq SponsoredPost.last
    end

    it "redirects to the new sponsored_post" do
      post :create, topic_id: my_topic.id, sponsored_post: {title: RandomData.random_sentence, body: RandomData.random_paragraph, price: rand(0..10)}
  ...
  describe "GET #edit" do
    ...
    it "assigns post to be updated to @sponsored_post" do
      ...
      expect(spost_instance.id).to eq my_sponsored_post.id
      expect(spost_instance.title).to eq my_sponsored_post.title
      expect(spost_instance.body).to eq my_sponsored_post.body
      expect(spost_instance.price).to eq my_sponsored_post.price
    end
  end
  ...
  describe "DELETE #destroy" do
    ...
    it "redirects to sponsored_post index" do
      delete :destroy, topic_id: my_topic.id, id: my_sponsored_post.id
